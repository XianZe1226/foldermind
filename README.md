...
当前桌面版不会依赖用户额外安装 `Python` 才能完成 PDF 提取和 OCR 请求。
...
当前 OCR 会按页补读，并优先保证扫描版 PDF 的完整性。
...
如果目标是让普通用户“下载安装就能用”，核心方案应该是原生安装包，而不是 Docker。

- macOS：发布 Apple Silicon `dmg`
- Windows：发布 x86_64 `nsis` 安装包
- 用户侧不需要额外安装 Python 或 Docker

Docker 更适合开发环境复现，不适合作为这个桌面应用的主要交付方式。
